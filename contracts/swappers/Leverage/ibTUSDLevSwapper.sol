...
contract ibTUSDLeverageSwapper {
    ...
    IERC20 public constant TUSD = IERC20(address(0));
    ...
    constructor(
        ...
        WBNB.approve(address(WBNB_TUSD_PAIR), type(uint256).max);
        TUSD.approve(address(TUSD_VAULT), type(uint256).max);
    }
    ...
    function swap(
        ...
        (reserve0, reserve1, ) = WBNB_TUSD.getReserves();
        ...
        balance = TUSD.balanceOf(address(this));

        TUSD_VAULT.deposit(balance);

        uint256 amountTo = TUSD_VAULT.balanceOf(address(this));

        (, shareReturned) = bentoBox.deposit(IERC20(address(TUSD_VAULT)), address(bentoBox), recipient, amountTo, 0);
