...
contract ibUSDTLeverageSwapper {
    ...
    IUniswapV2Pair public constant WBNB_USDT_PAIR = IUniswapV2Pair(address(0));
    AlpacaVault public constant USDT_VAULT = AlpacaVault(address(0));
    IWBNB public constant WBNB = IWBNB(address(0));

    // solhint-disable-next-line const-name-snakecase
    IERC20 public constant slUSD = IERC20(address(0));
    IERC20 public constant USDT = IERC20(address(0));
    ...
    constructor(
        ...
        WBNB.approve(address(WBNB_USDT_PAIR), type(uint256).max);
        USDT.approve(address(USDT_VAULT), type(uint256).max);
    }
    ...
    function swap(
        ...
        (reserve0, reserve1, ) = WBNB_USDT.getReserves();

        amountIntermediate = getAmountOut(balance, reserve0, reserve1);

        WBNB_USDT_PAIR.swap(0, amountIntermediate, address(this), new bytes(0));

        balance = USDT.balanceOf(address(this));

        USDT_VAULT.deposit(balance);

        uint256 amountTo = USDT_VAULT.balanceOf(address(this));
