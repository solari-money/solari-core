...
contract ibBNBLeverageSwapper {
    ...
    constructor(
        ...
        bentoBox = bentoBox_;
        WBNB.approve(address(BNB_VAULT), type(uint256).max);
    ...
    function swap(
        ...
        BNB_VAULT.deposit(balance);

        uint256 amountTo = BNB_VAULT.balanceOf(address(this));

        (, shareReturned) = bentoBox.deposit(IERC20(address(BNB_VAULT)), address(bentoBox), recipient, amountTo, 0);
