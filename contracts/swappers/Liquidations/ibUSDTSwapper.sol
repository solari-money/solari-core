...
contract ibUSDTSwapper is ISwapper {
    ...
    IUSDT public constant USDT = IUSDT(address(0));
    ...
    constructor(
        ...
        bentoBox = bentoBox_;
        USDT.approve(address(WBNB_USDT_PAIR), type(uint256).max);
    ...
    function swap(
        ...
        USDT_VAULT.withdraw(amountFrom);

        uint256 busdBalance = USDT.balanceOf(address(this));

        (uint256 reserve0, uint256 reserve1) = WBNB_USDT_PAIR.getReserves();
